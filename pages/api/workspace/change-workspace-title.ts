...
export default async function handler(req: NextApiRequest, res: NextApiResponse<string>) {
  try {
    ...
    const { workspaceId = undefined, newTitle = undefined } = { ...req.body };
    if (!workspaceId || typeof workspaceId !== "string")
      return res
        .status(400)
        .send("Workspace id to change title is missing or is not a non-empty string.");
    if (!newTitle || typeof newTitle !== "string")
      return res.status(400).send("Workspace new title is missing or is not a non-empty string.");
