...
export default interface GoalHistory {
    ...
    history: (HistoryAction<"title" | "description" | "authorId", string> | HistoryAction<"storyPoints", number> | HistoryAction<"objectives", {
        objective: string;
        done: boolean;
    }> | HistoryAction<"notes", {
        userId: string;
        note: string;
        date: Timestamp;
    }> | HistoryAction<"creationTime" | "modificationTime" | "deadline" | "placingInBinTime", Timestamp>)[];
    /**
     * @minLength 1
     */
    lastModifiedGoalId: string | null;
}
export const validateGoalHistory = (input: any): typia.IValidation<GoalHistory> => {
    const errors = [] as any[];
