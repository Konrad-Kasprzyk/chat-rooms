...
describe("Test userHistories collection firestore rules denying access.", () => {
  ...
  it(
    "The signed in user can't read a users history document from a workspace to which he does " +
      "not belong.",
    async () => {
      expect.assertions(1);
      ...
      await expect(
        getDoc(doc(collections.userHistories, workspace!.newestUsersHistoryId))
      ).rejects.toThrow(FirebaseError);
    }
  );

  it(
    "The signed in user can't get a list of users history documents, even if the list contains " +
      "only users history documents from the workspace to which he belongs.",
    async () => {
      expect.assertions(1);
      await signInTestUser(testUserId);
      await firstValueFrom(
      ...
      await expect(
        getDocs(
          query(
            ...
            where("workspaceId", "==", workspace!.newestUsersHistoryId)
          )
        )
      ).rejects.toThrow(FirebaseError);
    }
