...
describe("Test errors of uncovering a workspace invitation.", () => {
  ...
  /**
   * Creates the test workspace, invites the test user and hides the invitation.
   */
  beforeAll(async () => {
    ...
    await firstValueFrom(
      listenCurrentUserDetails().pipe(filter((userDetails) => userDetails?.id == invitedUser.uid))
    );
    ...
    await firstValueFrom(
      listenCurrentUserDetails().pipe(
        filter(
          (userDetails) =>
            userDetails?.id == invitedUser.uid &&
            userDetails.hiddenWorkspaceInvitationsIds.length == 1
        )
      )
    );
  }, BEFORE_ALL_TIMEOUT);
