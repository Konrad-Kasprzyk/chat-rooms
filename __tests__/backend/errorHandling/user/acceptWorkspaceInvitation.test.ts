...
describe("Test errors of accepting a workspace invitation.", () => {
  ...
  it("The document of the user using the api not found.", async () => {
    await testUserUsingApiNotFoundError(CLIENT_API_URLS.user.acceptWorkspaceInvitation);
  });
  ...
  it("The workspace document not found.", async () => {
    await testWorkspaceNotFoundError(CLIENT_API_URLS.user.acceptWorkspaceInvitation);
  });

  it("The users history document not found.", async () => {
    await testUsersHistoryNotFoundError(CLIENT_API_URLS.user.acceptWorkspaceInvitation);
  });

  it("The workspace is in the recycle bin.", async () => {
    await testWorkspaceInRecycleBinError(CLIENT_API_URLS.user.acceptWorkspaceInvitation);
  });

  it("The workspace has the deleted flag set.", async () => {
    await testWorkspaceHasDeletedFlagError(CLIENT_API_URLS.user.acceptWorkspaceInvitation);
  });
