...
const validateUserDetails = (input: any, errorFactory?: (p: import("typia").TypeGuardError.IProps) => Error): UserDetails => {
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is UserDetails => {
            ...
            const $ao0 = (input: any, _path: string, _exceptionable: boolean = true): boolean => ("string" === typeof input.id && (1 <= input.id.length || $guard(_exceptionable, {
                ...
                value: input.botNumber
            }, errorFactory)) && (5 === Object.keys(input).length || (false === _exceptionable || Object.keys(input).every((key: any) => {
                if (["id", "hiddenWorkspaceInvitationIds", "linkedUserDocumentIds", "mainUserId", "botNumber"].some((prop: any) => key === prop))
                    return true;
                const value = input[key];
                if (undefined === value)
