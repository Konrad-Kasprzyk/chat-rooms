...
const validateLabelsHistory = (input: any, errorFactory?: (p: import("typia").TypeGuardError.IProps) => Error): LabelsHistory => {
    const __is = (input: any, _exceptionable: boolean = true): input is LabelsHistory => {
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is LabelsHistory => {
            ...
            const $ao1 = (input: any, _path: string, _exceptionable: boolean = true): boolean => ("number" === typeof input.id || $guard(_exceptionable, {
                ...
                expected: "number",
                value: input.id
            }, errorFactory)) && ("created" === input.action || "modified" === input.action || "deleted" === input.action || $guard(_exceptionable, {
                path: _path + ".action",
