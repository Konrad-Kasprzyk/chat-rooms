 ...
 * overwritten.
 */
export default async function testArchivedTasksNotFoundError(
  ...
  await adminCollections.taskArchives.doc(workspace!.newestArchivedTasksId).delete();
  ...
  expect(await res.json()).toEqual(
    `Found the workspace document, but couldn't find the archived tasks document ` +
      `with id ${workspace!.newestArchivedTasksId}`
  );
}
