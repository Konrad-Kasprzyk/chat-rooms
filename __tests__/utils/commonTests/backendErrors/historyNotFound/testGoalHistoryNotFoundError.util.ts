 ...
 * overwritten.
 */
export default async function testGoalHistoryNotFoundError(
  ...
  // TODO create the goal. Check if goal with history exists after checking if user exists in backend.
  const goal = (await adminCollections.goals.doc("to-do").get()).data();
  await adminCollections.goalHistories.doc(goal!.newestHistoryId).delete();
  ...
  expect(await res.json()).toEqual(
    `Found the goal document, but couldn't find the goal history document ` +
      `with id ${goal!.newestHistoryId}`
  );
}
