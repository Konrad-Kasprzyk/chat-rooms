 ...
 * overwritten.
 */
export default async function testLabelsHistoryNotFoundError(
  ...
  await adminCollections.labelHistories.doc(workspace!.newestLabelsHistoryId).delete();
  ...
  expect(await res.json()).toEqual(
    `Found the workspace document, but couldn't find the labels history document ` +
      `with id ${workspace!.newestLabelsHistoryId}`
  );
}
